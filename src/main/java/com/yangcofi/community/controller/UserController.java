...
public class UserController implements CommunityConstant {
    ...
    public String uploadHeader(MultipartFile headerImage, Model model){     //接受前台传过来的文件
        ...
        String filename = headerImage.getOriginalFilename();          //原始文件名 用户传的时候文件叫什么名字就是什么名字
        String suffix = filename.substring(filename.lastIndexOf("."));
        if (StringUtils.isBlank(suffix)){
            model.addAttribute("error", "文件的格式有问题");
            return "/site/setting";
        }
        //生成随机文件名
        ...
        try {
            headerImage.transferTo(dest);       //把当前文件中的内容写入到文件中去了
        } catch (IOException e) {
            logger.error("上传文件失败: " + e.getMessage());
            throw new RuntimeException("上传文件失败，服务器发生异常!", e);
        }
        //跟新当前用户头像的路径 注意 这里提供的是WEB路径
    ...
    public void getHeader(@PathVariable("fileName") String fileName, HttpServletResponse response){
        ...
        //Java7语法 在try后面加上()，在小括号里面定义的变量，会自动在finally中关闭，前提是他有close()方法
        try (
                FileInputStream fis = new FileInputStream(fileName);  //输入流需要我们手动地去关闭
                OutputStream os = response.getOutputStream();         //输出流SpringMVC会自动关闭，因为是Responce的
        ){
